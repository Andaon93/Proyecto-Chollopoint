// 📄 Perfil.jsx
// Carga los chollos del usuario desde la API.
// Guarda los cambios del perfil vía API y sincroniza el AuthContext.

import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import {
  Box, Container, Paper, Typography, Avatar, Button,
  Divider, Chip, TextField,
  Stack, Alert, Tooltip, Tabs, Tab, LinearProgress,
...
export default function Perfil() {
  ...
  // ── Chollos del usuario cargados desde la API ─────────────────────────────
  ...
  // ── Datos del formulario inicializados desde el usuario del contexto ──────
  ...
  const progreso = sigNivel
    ? Math.round(((puntos - nivel.min) / (sigNivel.min - nivel.min)) * 100)
    : 100;
  ...
  // ── Guardar cambios del perfil vía API ────────────────────────────────────
  const guardarCambios = async () => {
    if (!tNombre.trim()) { setErrorForm("El nombre no puede estar vacío."); return; }
    if (tTelefono && !/^[0-9+ ]{7,15}$/.test(tTelefono)) {
      setErrorForm("El teléfono no tiene un formato válido."); return;
    }
    ...
    try {
      ...
      // Sincronizar con AuthContext para que el Navbar se actualice
  ...
  // ── Eliminar chollo desde el perfil ──────────────────────────────────────
  ...
  return (
    <Container maxWidth="md" sx={{ pt: 2, pb: 5 }}>
      ...
      {guardado && (
        <Alert severity="success" sx={{ mb: 3 }} onClose={() => setGuardado(false)}>
      ...
      <Paper elevation={4} sx={{ p: 3, borderRadius: 4, mb: 3 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 3, flexWrap: "wrap" }}>
          <Avatar sx={{ width: 80, height: 80, fontSize: 36,
            background: COLORES_AVATAR[colorAvatarIdx]?.bg || COLORES_AVATAR[0].bg }}>
            {inicial}
          </Avatar>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Stack direction="row" spacing={1} sx={{ mt: 1 }} flexWrap="wrap">
              <Chip
                ...
                size="small"
                sx={{ bgcolor: nivel.color + "22", color: nivel.color, fontWeight: "bold" }}
              />
              <Chip icon={<PersonIcon />} label="Verificado" color="success" size="small" />
              {usuario.ciudad && (
                <Chip icon={<LocationOnIcon />} label={usuario.ciudad} size="small" variant="outlined" />
              )}
            </Stack>
          </Box>
          ...
          <Stack spacing={1}>
            <Button
              ...
              sx={{
                ...
                "&:hover": { background: "linear-gradient(90deg, #c62828, #e64a19)" },
              }}
            >
              ✏️ Editar perfil
            </Button>
      ...
      <Paper elevation={2} sx={{ borderRadius: 4, overflow: "hidden" }}>
        <Tabs
          ...
          sx={{
            ...
            "& .MuiTab-root": { fontWeight: "bold", textTransform: "none", fontSize: 15 },
        ...
        <Box sx={{ p: 3 }}>
          ...
          <TabPanel value={tabActual} index={0}>
            ...
            <Stack spacing={2}>
              ...
              {usuario.bio && (
                <Box sx={{ display: "flex", alignItems: "flex-start", gap: 2 }}>
                  ...
                  <Box>
                    <Typography variant="caption" color="text.secondary">Sobre mí</Typography>
                    <Typography fontStyle="italic">"{usuario.bio}"</Typography>
                  </Box>
                </Box>
              )}
            ...
            {!usuario.ciudad && !usuario.fecha_nac && !usuario.telefono && !usuario.bio && (
              <Alert severity="info" sx={{ mt: 2 }}>
                Tu perfil está casi vacío — ve a <strong>Ajustes</strong> para añadir más información.
              </Alert>
            )}
            ...
            <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
              {[
                ...
                <Paper key={s.label} elevation={1}
                  sx={{ flex: 1, minWidth: 110, p: 2, borderRadius: 3, textAlign: "center" }}>
          ...
          <TabPanel value={tabActual} index={1}>
            ...
            <Paper variant="outlined" sx={{ p: 3, borderRadius: 3, mb: 3 }}>
              ...
              <LinearProgress
                ...
                sx={{
                  ...
                  bgcolor: "#eee",
                  "& .MuiLinearProgress-bar": { bgcolor: nivel.color, borderRadius: 5 },
                }}
              />
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                {[
                  ...
                  <Chip key={r.accion} label={`${r.accion} ${r.pts}`}
              ...
              {BADGES.map(badge => (
                <Paper
                  ...
                  sx={{
                    ...
                    opacity: badge.ganado ? 1 : 0.4,
                    border: badge.ganado ? "1.5px solid #ffa726" : "1px solid #ddd",
                    bgcolor: badge.ganado ? "#fffde7" : "transparent",
                  }}
                >
                  <Typography fontSize={28}>{badge.icono}</Typography>
                  <Typography fontWeight="bold" fontSize={13} sx={{ mt: 0.5 }}>
                    {badge.nombre}
                  </Typography>
                  <Typography variant="caption" color="text.secondary">{badge.desc}</Typography>
                  {!badge.ganado && (
                    <Chip label="Bloqueado" size="small" sx={{ mt: 1, fontSize: 10 }} />
                  )}
                </Paper>
              ))}
            </Box>

            {/* Mis chollos publicados */}
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
              ...
              <Button
                ...
                sx={{ bgcolor: "#e53935", color: "white", fontWeight: "bold",
            ...
            {cargandoChollos ? (
              ...
              <Paper variant="outlined" sx={{ p: 4, textAlign: "center", borderRadius: 3 }}>
                <Typography color="text.secondary" gutterBottom>
                  Todavía no has publicado ningún chollo.
                </Typography>
                <Button
                ...
                {misDeals.map(c => (
                  ...
                  >
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 1 }}>
                      <Box onClick={() => navigate(`/chollo/${c.id}`)}
                        ...
                        <Stack direction="row" spacing={1} sx={{ mt: 0.5 }} alignItems="center" flexWrap="wrap">
                          <Typography color="error" fontWeight="bold" fontSize={14}>
                            {c.precioOferta} €
                          </Typography>
                          {c.categoria && (
                            <Chip label={c.categoria} size="small" variant="outlined" />
                          )}
                          {c.ciudad && (
                            <Chip label={`📍 ${c.ciudad}`} size="small" variant="outlined" />
                          )}
                          {c.descuento && (
                            <Chip label={c.descuento} size="small" color="success" />
                          )}
                        </Stack>
                      </Box>
                      ...
                      <Stack direction="row" spacing={0.5} flexShrink={0}>
                        <Tooltip title="Editar chollo" arrow>
                          <IconButton size="small" onClick={() => navigate(`/editar/${c.id}`)}
                            sx={{ bgcolor: "#fff3e0", color: "#ff7043",
                        ...
                        <Tooltip title="Eliminar chollo" arrow>
                          <IconButton size="small" onClick={() => confirmarEliminar(c)}
                            sx={{ bgcolor: "#fce4ec", color: "#e53935",
                  ...
                  <Box key={cat.nombre}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
                      ...
                      <Typography variant="caption" color="text.secondary">
                        {cat.interacciones} interacciones
                      </Typography>
                    </Box>
                    <LinearProgress
                      variant="determinate"
          ...
          <TabPanel value={tabActual} index={2}>
            ...
            {errorForm && (
              <Alert severity="error" sx={{ mb: 3 }} onClose={() => setErrorForm("")}>
                {errorForm}
              </Alert>
            )}
            ...
            <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
              {COLORES_AVATAR.map((color, index) => (
                <Tooltip key={index} title={color.label} arrow>
                  <Box
                    onClick={() => setTColorAvatar(index)}
                    sx={{
                      width: 36, height: 36, borderRadius: "50%",
                      background: color.bg, cursor: "pointer",
                      border: tColorAvatar === index ? "3px solid #222" : "3px solid transparent",
            ...
            <Avatar sx={{ width: 60, height: 60, fontSize: 26,
              background: COLORES_AVATAR[tColorAvatar].bg, mb: 3 }}>
            ...
            <Stack spacing={2.5}>
              ...
              <TextField
                label="Nombre completo" value={tNombre}
                onChange={e => setTNombre(e.target.value)}
                fullWidth required
                error={!tNombre.trim()}
                helperText={!tNombre.trim() ? "Campo obligatorio" : ""}
              />

              <TextField
                label="Alias (nombre de usuario)" value={tAlias}
                onChange={e => setTAlias(e.target.value)}
                fullWidth placeholder="Ej: cazachollos99"
                inputProps={{ maxLength: 30 }}
                helperText={`${tAlias.length}/30`}
              />

              <TextField
                label="Biografía" value={tBio}
                onChange={e => setTBio(e.target.value)}
                fullWidth multiline rows={3}
                placeholder="Cuéntanos algo sobre ti..."
                inputProps={{ maxLength: 160 }}
                helperText={`${tBio.length}/160`}
              />
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                <TextField
                  label="Ciudad" value={tCiudad}
                  onChange={e => setTCiudad(e.target.value)}
                  fullWidth placeholder="Ej: Sevilla"
                />
                <FormControl fullWidth>
                  ...
                  <Select
                    value={tProvincia}
                    label="Provincia"
                    onChange={e => setTProvincia(e.target.value)}
                    renderValue={(v) => v || "Seleccionar"}
              ...
              <TextField
                ...
                value={tFechaNac} onChange={e => setTFechaNac(e.target.value)}
                fullWidth InputLabelProps={{ shrink: true }}
                inputProps={{ max: new Date().toISOString().split("T")[0] }}
              />

              <TextField
                label="Teléfono de contacto" value={tTelefono}
                onChange={e => setTTelefono(e.target.value)}
                fullWidth placeholder="Ej: +34 612 345 678"
                helperText="Solo visible para ti"
              />
              ...
              <TextField
                ...
                fullWidth disabled helperText="El correo no se puede cambiar desde aquí"
              />
            </Stack>
            ...
            <Button
              ...
              disabled={!tNombre.trim() || cargandoPerfil}
      ...
      <Dialog open={!!dealAEliminar} onClose={() => setDealAEliminar(null)}>
        ...
        <DialogContent>
          <DialogContentText>
            Vas a eliminar <strong>"{dealAEliminar?.titulo}"</strong>.
