// 📄 PublicarChollo.jsx
// Añadida sección de ubicación exacta:
//   - Campos: calle, número, CP, ciudad
//   - Botón "Buscar ubicación" → llama a Nominatim (OpenStreetMap) y guarda lat/lng
//   - Las coordenadas se envían al backend junto al resto del formulario
 ...
 */
const geocodificar = async (calle, numero, cp, ciudad) => {
  ...
  const res = await fetch(url, {
    headers: { "Accept-Language": "es" }, // respuestas en español
  });
  ...
  if (data.length > 0) {
    return {
      ...
      direccion: data[0].display_name, // dirección formateada que devuelve Nominatim
    };
  }
  return null;
};
...
export default function PublicarChollo() {
  ...
  // Estado para la sección de ubicación exacta
  const [ubicacion, setUbicacion] = useState({
    calle: "", numero: "", cp: "", ciudad: "",
  });
  const [coords,          setCoords]          = useState(null);   // { lat, lng, direccion }
  ...
  const cambiarUbicacion = (campo) => (e) =>
  ...
  const buscarUbicacion = async () => {
    setErrorUbicacion("");
    if (!ubicacion.calle.trim() || !ubicacion.ciudad.trim()) {
      setErrorUbicacion("Introduce al menos la calle y la ciudad.");
      return;
    }
    ...
    try {
      ...
      if (resultado) {
        ...
        setErrorUbicacion("No se encontró esa dirección. Prueba con menos datos o comprueba el nombre.");
        setCoords(null);
      }
    } catch {
      setErrorUbicacion("Error al buscar la ubicación. Comprueba tu conexión.");
      setCoords(null);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.chollos.crear({
        ...
        // ── Coordenadas exactas (si el usuario las buscó) ──
      ...
      if (err.message === 'SESSION_EXPIRED') {
        sesionExpirada(); // limpia también el estado React (avatar desaparece del Navbar)
        setErrorApi('SESSION_EXPIRED');
      } else {
  ...
  return (
    <Container maxWidth="xl" sx={{ pt: 2, pb: 4 }}>
      ...
      {errorApi && (
        ...
        >
          {errorApi === 'SESSION_EXPIRED'
            ? "Tu sesión ha expirado. Por favor, vuelve a iniciar sesión para publicar el chollo."
            : errorApi}
        </Alert>
      )}
      ...
      <Grid container spacing={3} alignItems="flex-start">
        ...
        <Grid size={{ xs: 12, lg: 7 }}>
          <Stack spacing={2.5}>
            ...
            {/* ── SECCIÓN NUEVA: Ubicación exacta ── */}
            <Paper elevation={2} sx={{ p: 3, borderRadius: 3 }}>
              <SeccionHeader
                icon={<PlaceIcon fontSize="small" />}
                titulo="Ubicación exacta (opcional)"
                subtitulo="Si el chollo es en una tienda física, añade la dirección para que aparezca en el mapa"
              />
              <Divider sx={{ mb: 2.5 }} />
              ...
              <Stack spacing={2}>
                ...
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                  <TextField
                    ...
                    onChange={cambiarUbicacion("calle")}
                  />
                  <TextField
                    label="Número"
                    placeholder="Ej: 32"
                    value={ubicacion.numero}
                    onChange={cambiarUbicacion("numero")}
                ...
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                  ...
                  <TextField
                    ...
                    onChange={cambiarUbicacion("ciudad")}
                  />
                </Stack>

                {/* Error de geocodificación */}
                {errorUbicacion && (
                  <Alert severity="warning" onClose={() => setErrorUbicacion("")}>
                    {errorUbicacion}
                  </Alert>
                )}
                ...
                <Button
                  variant="outlined"
                  startIcon={buscandoCoords ? <CircularProgress size={16} /> : <SearchIcon />}
                  ...
                  sx={{
                    ...
                    fontWeight: "bold",
                    borderRadius: 3,
                    "&:hover": { bgcolor: "#fce4ec", borderColor: "#c62828" },
                  }}
                >
            ...
            <Paper elevation={2} sx={{ borderRadius: 3, overflow: "hidden" }}>
              ...
              <Box sx={{ p: 3 }}>
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mb: 2 }}>
                  ...
                  {/* Badge de ubicación exacta si ya se buscó */}
                  {coords && (
                    <Chip
                      icon={<PlaceIcon />}
                ...
                {/* Mostrar dirección encontrada en el preview */}
                {coords && (
