...
--
--  Este archivo refleja exactamente la base de datos en producción.
--  Si ya tienes las tablas creadas, mira el bloque MIGRACIÓN al final.
-- ============================================================
...
-- ── Tabla de usuarios ─────────────────────────────────────────────────────────
-- Guarda todos los usuarios registrados en CholloPoint
CREATE TABLE IF NOT EXISTS `usuarios` (
    `id`              INT UNSIGNED      NOT NULL AUTO_INCREMENT,
    `nombre`          VARCHAR(100)      NOT NULL,
    `alias`           VARCHAR(50)       DEFAULT NULL,
    `email`           VARCHAR(191)      NOT NULL,
    `password_hash`   VARCHAR(255)      NOT NULL,                -- La contraseña siempre cifrada, nunca en texto plano
    `avatar`          CHAR(1)           NOT NULL DEFAULT 'U',    -- Primera letra del nombre en mayúscula
    ...
    `color_avatar`    TINYINT UNSIGNED  NOT NULL DEFAULT '0',    -- Número que representa el color del avatar en el frontend
...
-- ── Tabla de chollos ──────────────────────────────────────────────────────────
-- Guarda todos los chollos publicados en CholloPoint
CREATE TABLE IF NOT EXISTS `chollos` (
    ...
    `descuento`        VARCHAR(10)    DEFAULT NULL,              -- Ej: "-47%"
    ...
    -- Ubicación exacta del chollo (opcional)
    -- Se rellena cuando el usuario escribe la dirección al publicar el chollo
    -- Si es NULL, el chollo no aparece con marcador exacto en el mapa
    ...
    `expira_en`        DATETIME       NOT NULL,                  -- Se calcula como creado_en + 24 horas
    ...
    KEY `idx_coords`    (`latitud`, `longitud`)                  -- Para filtrar rápido chollos con ubicación exacta
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;


-- ── Tabla de comentarios ──────────────────────────────────────────────────────
-- Guarda los comentarios que los usuarios escriben en los chollos
CREATE TABLE IF NOT EXISTS `comentarios` (
    `id`               INT UNSIGNED  NOT NULL AUTO_INCREMENT,
    `chollo_id`        INT UNSIGNED  NOT NULL,
    `usuario_id`       INT UNSIGNED  NOT NULL,
    `texto`            TEXT          NOT NULL,
    `votos`            INT           NOT NULL DEFAULT '0',       -- Columna heredada (no se usa activamente, se mantiene por compatibilidad)
    `creado_en`        DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `votos_positivos`  INT           NOT NULL DEFAULT '0',       -- Votos positivos recibidos
    `votos_negativos`  INT           NOT NULL DEFAULT '0',       -- Votos negativos recibidos
    PRIMARY KEY (`id`),
...
-- ── Tabla de favoritos ────────────────────────────────────────────────────────
-- Guarda los chollos que cada usuario ha marcado como favorito
-- La clave única (usuario_id + chollo_id) impide que el mismo usuario guarde el mismo chollo dos veces
...
-- ── Tabla de votos en chollos ─────────────────────────────────────────────────
-- Registra quién votó qué chollo y de qué tipo fue el voto
-- La clave única (chollo_id + usuario_id) impide que el mismo usuario vote dos veces el mismo chollo
...
-- Registra quién votó qué comentario y de qué tipo fue el voto
-- La clave única (comentario_id + usuario_id) impide que el mismo usuario vote dos veces el mismo comentario
...
-- ── Claves foráneas ───────────────────────────────────────────────────────────
-- Las claves foráneas garantizan que no haya datos huérfanos:
-- si se borra un usuario, se borran también sus chollos, comentarios, votos y favoritos
...
-- ── Datos de ejemplo ──────────────────────────────────────────────────────────
-- Dos usuarios y dos chollos de prueba para que la app no aparezca vacía al instalarse
-- INSERT IGNORE significa que si ya existen (mismo ID) no da error, simplemente los ignora
...
-- ============================================================
--  MIGRACIÓN: Si ya tienes las tablas creadas y solo quieres
--  añadir columnas que falten, ejecuta solo el bloque que necesites.
--  (Quita los -- del principio de cada línea para activarlo)
-- ============================================================

-- Añadir columnas de ubicación a chollos (si no las tienes aún):
-- ALTER TABLE `chollos`
...
-- Añadir columnas de votos separados a comentarios (si solo tienes la columna `votos`):
-- ALTER TABLE `comentarios`
--     ADD COLUMN `votos_positivos` INT NOT NULL DEFAULT '0' AFTER `creado_en`,
--     ADD COLUMN `votos_negativos` INT NOT NULL DEFAULT '0' AFTER `votos_positivos`;