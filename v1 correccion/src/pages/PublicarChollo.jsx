...
export function leerDeals() {
  try { const raw = localStorage.getItem("chollopoint_deals"); if (raw) return JSON.parse(raw); return []; } catch { return []; }
}

export function eliminarDeal(id) {
  const filtrados = leerDeals().filter((d) => String(d.id) !== String(id));
  localStorage.setItem("chollopoint_deals", JSON.stringify(filtrados));
}

export function estaExpirado(deal) {
  if (deal.expiraEn) return Date.now() > deal.expiraEn;
  if (deal.creadoEn) return Date.now() - deal.creadoEn > 24 * 60 * 60 * 1000;
  return false;
}

async function geocodificar(calle, numero, cp, ciudad) {
  const direccion = `${calle} ${numero}, ${cp} ${ciudad}, España`;
  const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(direccion)}&format=json&limit=1`;
  const res  = await fetch(url, { headers: { "Accept-Language": "es" } });
  const data = await res.json();
  if (data.length > 0) return { lat: parseFloat(data[0].lat), lng: parseFloat(data[0].lon), direccion: data[0].display_name };
  return null;
}
...
export default function PublicarChollo() {
  ...
  const [ubicacion, setUbicacion] = useState({ calle: "", numero: "", cp: "", ciudad: "" });
  ...
  function cambiar(campo) { return (e) => setForm((prev) => ({ ...prev, [campo]: e.target.value })); }
  function cambiarUbicacion(campo) { return (e) => setUbicacion((prev) => ({ ...prev, [campo]: e.target.value })); }
  ...
  async function buscarUbicacion() {
    setErrorUbicacion("");
    if (!ubicacion.calle.trim() || !ubicacion.ciudad.trim()) { setErrorUbicacion("Introduce al menos la calle y la ciudad."); return; }
    setBuscandoCoords(true);
    try {
      const resultado = await geocodificar(ubicacion.calle, ubicacion.numero, ubicacion.cp, ubicacion.ciudad);
      if (resultado) { setCoords(resultado); }
      else { setErrorUbicacion("No se encontró esa dirección. Prueba con menos datos o comprueba el nombre."); setCoords(null); }
    } catch {
      setErrorUbicacion("Error al buscar la ubicación. Comprueba tu conexión."); setCoords(null);
    }
  ...
  async function handleSubmit() {
    ...
    try {
      await api.chollos.crear({
        ...
        direccion_exacta: coords?.direccion ?? null,
      });
      setEnviado(true);
      setTimeout(() => navigate("/"), 1500);
    } catch (err) {
      if (err.message === "SESSION_EXPIRED") { sesionExpirada(); setErrorApi("SESSION_EXPIRED"); }
      else { setErrorApi(err.message || "Error al publicar el chollo"); }
    }
    setCargando(false);
  }
    ...
    return (
      <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh" }}>
        <Paper sx={{ p: 5, textAlign: "center", borderRadius: 4, maxWidth: 400 }}>
          <Typography variant="h5" gutterBottom fontWeight="bold">Inicia sesión para publicar</Typography>
          <Typography color="text.secondary" sx={{ mb: 3 }}>Solo los usuarios registrados pueden publicar chollos.</Typography>
    ...
    return (
      <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "60vh" }}>
        <Paper sx={{ p: 5, textAlign: "center", borderRadius: 4, maxWidth: 400 }}>
          ...
          <Typography color="text.secondary">Tu chollo ya está visible. Caduca en <strong>24 horas</strong>.</Typography>
          <Typography variant="caption" color="text.secondary" display="block" sx={{ mt: 1 }}>Redirigiendo al inicio...</Typography>
        </Paper>
      </Box>
    );
  }
  ...
  return (
    <Container maxWidth="xl" sx={{ pt: 2, pb: 4 }}>
      <Box sx={{ mb: 3 }}>
        <Typography variant="h4" fontWeight="bold" gutterBottom>🔥 Publicar un chollo</Typography>
        <Alert severity="info" icon={<AccessTimeIcon />} sx={{ maxWidth: 600 }}>
          Los chollos se marcan como <strong>expirados</strong> automáticamente pasadas <strong>24 horas</strong> desde su publicación.
        </Alert>
      </Box>

      {errorApi && (
        <Alert severity="error" sx={{ mb: 2 }} onClose={() => setErrorApi("")}
          action={errorApi === "SESSION_EXPIRED" ? <Button color="inherit" size="small" onClick={() => navigate("/login")}>Volver a entrar</Button> : undefined}>
          {errorApi === "SESSION_EXPIRED" ? "Tu sesión ha expirado. Por favor, vuelve a iniciar sesión para publicar el chollo." : errorApi}
        </Alert>
      )}
      ...
      <Grid container spacing={3} alignItems="flex-start">
        <Grid size={{ xs: 12, lg: 7 }}>
          <Stack spacing={2.5}>
            <Paper elevation={2} sx={{ p: 3, borderRadius: 3 }}>
              ...
              <Stack spacing={2.5}>
                <TextField fullWidth label="Título del chollo *" value={form.titulo} onChange={cambiar("titulo")}
                  error={!!errores.titulo} helperText={errores.titulo || `${form.titulo.length}/100 caracteres`}
                  inputProps={{ maxLength: 100 }} placeholder='Ej: TV Samsung 55" 4K QLED a mitad de precio' />
            ...
            <Paper elevation={2} sx={{ p: 3, borderRadius: 3 }}>
              <SeccionHeader icon={<EuroIcon fontSize="small" />} titulo="Precios" subtitulo="El descuento se calcula automáticamente" />
              <Divider sx={{ mb: 2.5 }} />
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            ...
            <Paper elevation={2} sx={{ p: 3, borderRadius: 3 }}>
              <SeccionHeader icon={<StoreIcon fontSize="small" />} titulo="Tienda e imagen" subtitulo="¿Dónde se puede conseguir? Añade un enlace y una foto" />
              <Divider sx={{ mb: 2.5 }} />
              <Stack spacing={2.5}>
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                  ...
                  <TextField fullWidth label="Enlace a la oferta" value={form.enlace} onChange={cambiar("enlace")}
                    placeholder="https://..." helperText="Opcional — URL directa a la oferta"
            ...
            <Paper elevation={2} sx={{ p: 3, borderRadius: 3 }}>
              <SeccionHeader icon={<PlaceIcon fontSize="small" />} titulo="Ubicación exacta (opcional)" subtitulo="Si el chollo es en una tienda física, añade la dirección para que aparezca en el mapa" />
              <Divider sx={{ mb: 2.5 }} />
              <Stack spacing={2}>
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                  <TextField fullWidth label="Calle" placeholder="Ej: Calle Gran Vía" value={ubicacion.calle} onChange={cambiarUbicacion("calle")} />
                  <TextField label="Número" placeholder="Ej: 32" value={ubicacion.numero} onChange={cambiarUbicacion("numero")} sx={{ width: { xs: "100%", sm: 120 } }} />
                </Stack>
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                  <TextField label="Código postal" placeholder="Ej: 28013" value={ubicacion.cp} onChange={cambiarUbicacion("cp")} sx={{ width: { xs: "100%", sm: 160 } }} />
                  <TextField fullWidth label="Ciudad / Municipio" placeholder="Ej: Madrid" value={ubicacion.ciudad} onChange={cambiarUbicacion("ciudad")} />
                </Stack>
                {errorUbicacion && <Alert severity="warning" onClose={() => setErrorUbicacion("")}>{errorUbicacion}</Alert>}
                {coords && (
                  <Alert severity="success" icon={<CheckCircleIcon />}>
                    <strong>Ubicación encontrada:</strong> {coords.direccion}
                    <Typography variant="caption" display="block" sx={{ mt: 0.5, color: "text.secondary" }}>
                      Coordenadas: {coords.lat.toFixed(5)}, {coords.lng.toFixed(5)}
                    </Typography>
                  </Alert>
                )}
                <Button variant="outlined" startIcon={buscandoCoords ? <CircularProgress size={16} /> : <SearchIcon />}
                  onClick={buscarUbicacion} disabled={buscandoCoords}
                  sx={{ alignSelf: "flex-start", borderColor: "#e53935", color: "#e53935", fontWeight: "bold", borderRadius: 3, "&:hover": { bgcolor: "#fce4ec", borderColor: "#c62828" } }}>
                  {buscandoCoords ? "Buscando..." : "Buscar ubicación en el mapa"}
                </Button>
                <Typography variant="caption" color="text.secondary">
                  💡 Si la búsqueda no encuentra tu dirección exacta, prueba poniendo solo la calle y la ciudad.
                </Typography>
              </Stack>
            </Paper>

            <Box sx={{ display: "flex", gap: 2 }}>
              <Button variant="contained" color="error" size="large" startIcon={<SendIcon />}
                onClick={handleSubmit} disabled={cargando} sx={{ fontWeight: "bold", px: 5, borderRadius: 3 }}>
                {cargando ? "Publicando..." : "Publicar chollo"}
              </Button>
              <Button variant="outlined" size="large" onClick={() => navigate("/")} sx={{ borderRadius: 3 }}>Cancelar</Button>
            </Box>
          </Stack>
        </Grid>
            ...
            <Paper elevation={2} sx={{ borderRadius: 3, overflow: "hidden" }}>
              ...
              <Box sx={{ p: 3 }}>
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ mb: 2 }}>
                  ...
                  {coords && <Chip icon={<PlaceIcon />} label="Ubicación exacta ✓" size="small" color="success" sx={{ fontWeight: "bold" }} />}
                </Stack>
                ...
                {coords && (
                  <Stack direction="row" spacing={1} alignItems="flex-start" sx={{ mt: 1 }}>
                    <PlaceIcon fontSize="small" sx={{ color: "#e53935", mt: 0.3 }} />
                    <Typography variant="caption" color="text.secondary">
                      {ubicacion.calle} {ubicacion.numero}, {ubicacion.cp} {ubicacion.ciudad}
                    </Typography>
                  </Stack>
                )}
